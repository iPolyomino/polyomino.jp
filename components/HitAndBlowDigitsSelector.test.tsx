...
describe("HitAndBlowDigitsSelector", () => {
  it("正しい桁数オプション（3,4,5）が表示される", () => {
    render(
      <HitAndBlowDigitsSelector digit={3} handleNumberLength={jest.fn()} />,
    );
  ...
  it("初期値に対応するボタンが選択状態になっている", () => {
    render(
      <HitAndBlowDigitsSelector digit={4} handleNumberLength={jest.fn()} />,
    );
    ...
    expect(screen.getByText("5").closest("button")).toHaveAttribute(
      "aria-pressed",
      "false",
    );
  });
